// app/edit-transaction/[id].tsx — Edit transaction: same form as add, pre-filled, Update button.
...
export default function EditTransactionScreen() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const tx = txRes.data
      const type = (tx.entry_type === "INCOME" || tx.entry_type === "EXPENSES" || tx.entry_type === "CONTRA"
        ? tx.entry_type
        : "EXPENSES") as TxType
      setTxType(type)
      setFromAccountId(tx.from_account_id ?? defaultExpenseId)
      setToAccountId(tx.to_account_id ?? defaultIncomeId)
  ...
  const handleSubmit = async () => {
    ...
    const { error } = await updateTransaction(userId, id, {
...
const styles = StyleSheet.create({
  ...
  typeChipTextSelected: { color: AppColors.black },
  chipRow: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 10,
  ...
  chipTextSelected: { color: AppColors.black },
  addCategoryCircle: {
    width: 44,
    height: 44,
    borderRadius: 22,
